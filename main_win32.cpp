...
static LRESULT CALLBACK wndProc(HWND window, UINT message, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_CREATE: {
        ...
        window_handle.hinstance = create_info->hInstance;
        window_handle.hwnd = window;

        std::string out_error_message;
#ifdef DEBUG
        if (!user_data->renderer.init(out_error_message, window_handle, vulkanDebugCallback, &(user_data->logger))) {
#else
        if (!user_data->renderer.init(out_error_message, window_handle)) {
#endif
