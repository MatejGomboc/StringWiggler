/**
 * StringWiggler GitHub Actions Cache Cleanup Script
 *
 * Keeps only the most recent cache per group (vulkan-sdk-windows, vulkan-sdk-linux,
 * npm-markdownlint-Linux, etc.) and deletes all older/stale caches.
 */
...
module.exports = async ({ github, context, core }) => {
    ...
    try {
        log("=".repeat(60));
        log("StringWiggler Actions Cache Cleanup");
        log("=".repeat(60));
        log("");
        ...
        // Vulkan SDK caches: vulkan-sdk-{OS}-{version}
        ...
        for (const cache of caches) {
            ...
            if (cache.key.startsWith("vulkan-")) {
                ...
                // Unknown cache type, use the full key as prefix (won't group).
                prefix = cache.key;
            }
