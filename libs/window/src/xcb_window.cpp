...
namespace WindowLib
{
    ...
    void XcbWindow::setCursorCaptured(bool captured)
    {
        ...
        if (captured) {
            // Create an invisible cursor. Per the X11 spec, freshly-created pixmap contents
            // are undefined, so on some servers an uninitialised "invisible" cursor would
            // render as a visible single pixel. Explicitly zero the pixmap with a GC fill so
            // the cursor is guaranteed transparent.
            ...
            // Build the cursor from the (now zeroed) pixmap. With the mask all-zero, every
            // pixel is transparent regardless of source — so passing the same zeroed pixmap
            // for both arguments yields a fully invisible cursor.
